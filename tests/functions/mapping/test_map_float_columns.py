"""Tests for map_float_columns function.
...
from tubular.functions.mapping import map_float_columns
...
def test_warning(verbose, recwarn):
    ...
    msg = """map_float_columns: Note if working in pandas and casting to Boolean,
    ...
    if not verbose:
        assert ~any(msg in str(w.message) for w in recwarn), (
            "unexpected warning raised from map_float_columns"
        )

    else:
        assert any(msg in str(w.message) for w in recwarn), (
            "expected warning not raised from map_float_columns"
        )
