"""Tests for map_string_columns function.
...
from tubular.functions.mapping import map_string_columns
...
def test_warning(verbose, recwarn):
    ...
    msg = """map_string_columns: Note if working in pandas and casting to Boolean,
    ...
    if not verbose:
        assert ~any(msg in str(w.message) for w in recwarn), (
            "unexpected warning raised from map_string_columns"
        )

    else:
        assert any(msg in str(w.message) for w in recwarn), (
            "expected warning not raised from map_string_columns"
        )
